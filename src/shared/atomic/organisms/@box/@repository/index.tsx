...
import { useQueryProfileGaragesHook } from "@/features/user/core/hooks/profile.hooks";
import { useProfileGarageStore } from "@/features/user/infrastructure/atoms/profile.atoms";
    ...
    <S.Container>
      ...
      <S.Grid>
        {garages.map((garage, index) => (
          <S.RepoCard key={index} href={garage.url ? garage.url : ""} onClick={e => handleRepoClick(e, garage)}>
            <S.RepoHeader>
          ...
          <S.ModalContent>
            ...
            <S.ModalBody>
              ...
              <S.ModalSection>
                ...
                <S.TagList>{selectedRepo.details?.technologies?.map((tech, index) => <S.Tag key={index}>{tech}</S.Tag>)}</S.TagList>
              </S.ModalSection>

              <S.ModalSection>
                <S.SectionTitle>Key Features</S.SectionTitle>
                <S.FeatureList>
                  {selectedRepo.details?.features?.map((feature, index) => <S.FeatureItem key={index}>{feature}</S.FeatureItem>)}
                </S.FeatureList>
              </S.ModalSection>
            </S.ModalBody>
          </S.ModalContent>
